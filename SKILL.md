...
#### 2. 模拟演示模式 (降级回滚)
- **触发条件**: 上述条件不满足
- **行为**: 单个 AI 助手分饰多角，**必须在每个步骤后显式执行保存代码**。

---

### 🔴 下文指南针对：模拟演示模式 (Fallback)

**如果处于模拟模式，核心原则**:
1. **持久化必须手动触发**: 不依赖系统自动保存。
2. **分析完成后立即执行**: 完成一个 Agent 的工作后，立即运行保存脚本。
...
### 🟢 生产模式部署
请参阅 `docs/persistence_guide.md` 中的生产环境部署章节。
